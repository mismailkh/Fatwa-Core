...
{
    public partial class ListCrashReport : ComponentBase
    {
        ...
        public bool IsCrashReport { get; set; } = true;
        ...
        /*<History Author='Ammaar Naveed' Date='2025-02-06' Version="1.0" Branch="master">Populate grid data based on pagination</History>*/
        protected async Task OnLoadData(LoadDataArgs args)
        {
            try
            {
                if (string.IsNullOrEmpty(args.OrderBy) || CurrentPage != advanceSearchBugList.PageNumber || CurrentPageSize != advanceSearchBugList.PageSize || (Keywords && advanceSearchBugList.isDataSorted))
                {
                    ...
                    spinnerService.Show();
                    advanceSearchBugList.IssueTypeId = (int)IssueTypeEnum.SystemCrashing;
        ...
        {
            navigationManager.NavigateTo("/reportedbug-view/" + bug.Id + "/" + IsCrashReport);
        }
        ...
        protected async Task OnSearchInput(string value)
        {
            try
            {
                    ...
                    FilteredBugListVM = await gridSearchExtension.Filter(GetBugs, new Query()
                {
                    ...
                    (i.PrimaryBugId != null && i.PrimaryBugId.ToString().ToLower().Contains(@0)) || 
                    (i.ApplicationEn != null && i.ApplicationEn.ToString().ToLower().Contains(@1)) || 
                    (i.ModuleEn != null && i.ModuleEn.ToString().ToLower().Contains(@2))  || 
                    (i.StatusEn != null && i.StatusEn.ToString().ToLower().Contains(@3)) || 
                    (i.CreatedDate != null && i.CreatedDate.ToString(""dd/MM/yyyy"").Contains(@4)) ||
                    (i.Subject != null && i.Subject.ToString().ToLower().Contains(@5)) || 
                    (i.IssueTypeEn != null && i.IssueTypeEn.ToString().ToLower().Contains(@6))" 
                    :  
                    ...
                    (i.StatusAr != null && i.StatusAr.ToString().ToLower().Contains(@3)) || 
                    (i.CreatedDate != null && i.CreatedDate.ToString(""dd/MM/yyyy"").Contains(@4))
                    (i.Subject != null && i.Subject.ToString().ToLower().Contains(@5)) || 
                    (i.IssueTypeAr != null && i.IssueTypeAr.ToString().ToLower().Contains(@6))",
