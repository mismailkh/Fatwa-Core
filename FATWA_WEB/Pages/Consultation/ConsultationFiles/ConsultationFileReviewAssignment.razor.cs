...
{
    public partial class ConsultationFileReviewAssignment : ComponentBase
    {
        #region Parameter
        [Parameter]
        ...
        protected ConsultationAssignment? getconsultationAssignment { get; set; }
        ...
        {
            var dialogResult = await dialogService.OpenAsync<ApproveRejectAssignmentConsultationFile>
                (
                translationState.Translate("Approve_Reject_ConsultationFile"),
                new Dictionary<string, object>() {
                    { "ReferenceId", Guid.Parse(FileId) },
                    { "TaskId", TaskId }

                },
                new DialogOptions() { Width = "30% !important",Height= "20% !important", CloseDialogOnOverlayClick = true });
        ...
        protected async Task PopulateConsultationFileFileAssigmentbyFileIdAndLawyerId()
        {
            var userresponse = await comsSharedService.GetConsultationAssigmentByLawyerIdAndFileId(Guid.Parse(FileId), loginState.UserDetail.UserId);
            if (userresponse.IsSuccessStatusCode)
            {
                getconsultationAssignment = (ConsultationAssignment)userresponse.ResultData;
            }
            else
            {
