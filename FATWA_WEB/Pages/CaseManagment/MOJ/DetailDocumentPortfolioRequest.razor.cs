...
{
    public partial class DetailDocumentPortfolioRequest : ComponentBase
    {
        ...
        protected MojRequestForDocument requestForDocument { get; set; } = new MojRequestForDocument();
        ...
        {
            spinnerService.Show();
            await PopulateRequestForDocument();
        ...
        protected async Task PopulateAttachementsGrid()
        {
            try
            {
                attachments = await fileUploadService.GetTempAttachements(requestForDocument.Id);
        ...
        //<History Author = 'Hassan Abbas' Date='2023-03-24' Version="1.0" Branch="master">Populate Request For Document</History>
        protected async Task PopulateRequestForDocument()
        {
            var response = await cmsRegisteredCaseService.GetRequestForDocumentById(Guid.Parse(Id));
            if (response.IsSuccessStatusCode)
            {
                requestForDocument = (MojRequestForDocument)response.ResultData;
                documentPortfolio.Name = "Document_Portfolio_" + requestForDocument.HearingDate.ToString("dd-MM-yyyy");
                requestForDocument.Id = Guid.NewGuid();
            }
            else
            {
                await invalidRequestHandlerService.ReturnBadRequestNotification(response);
        ...
        //<History Author = 'Hassan Abbas' Date='2023-07-05' Version="1.0" Branch="master"> Modified the function to Decrypt the attachment and then view</History>
        ...
        protected async Task SubmitPortfolio()
        {
            ...
            }) == true)
            {
                ...
                ApiCallResponse response1 = new ApiCallResponse();
                documentPortfolio.ReferenceId = requestForDocument.CaseId;
                ...
                if (response.IsSuccessStatusCode)
                {
                    var user = (CmsDocumentPortfolio)response.ResultData;
                    response = await cmsRegisteredCaseService.UpdateDocumentPortfolioRequest(Guid.Parse(Id));
                    //response1 = await cmsRegisteredCaseService.GetRequestForDocumentById(Guid.Parse(Id));
                    //var res1 = (MojRequestForDocument)response1.ResultData;

                    if (response.IsSuccessStatusCode)
                    {

                        //  response = await cmsSharedService.UserIdByUserEmail(documentPortfolio.CreatedBy);
                        //var res =  (User)response.ResultData;
                        Guid Refernceid = Guid.Parse(Id);
                        var result = await taskService.TaskUpdateWithService(Refernceid);
                        notificationService.Notify(new NotificationMessage()
                        {
        ...
        //<History Author = 'Hassan Abbas' Date='2023-03-24' Version="1.0" Branch="master"> Open popup for uploading document in the same component</History>
        protected async Task AddDocument()
        {
            var result = await dialogService.OpenAsync<FileUpload>(translationState.Translate("Upload_Documents"),
                ...
                {
                    { "ReferenceGuid", requestForDocument.Id },
                    { "IsViewOnly", false },
                    { "IsUploadPopup", true },
                    ...
                    { "ModuleId", (int)WorkflowModuleAndActivitiesEnum.WorkflowModuleEnum.CaseManagement },
                    { "MandatoryAttachmentTypeId", (int)AttachmentTypeEnum.DocumentPortfolio },
                    { "IsCaseRequest", false },
            ...
            if (document != null)
            {
                document.Type_En = AttachmentTypes.Where(t => t.AttachmentTypeId == document.AttachmentTypeId).FirstOrDefault()?.Type_En;
                document.Type_Ar = AttachmentTypes.Where(t => t.AttachmentTypeId == document.AttachmentTypeId).FirstOrDefault()?.Type_Ar;
