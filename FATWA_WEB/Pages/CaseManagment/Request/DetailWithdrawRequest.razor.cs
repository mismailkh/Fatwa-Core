...
{
    public partial class DetailWithdrawRequest : ComponentBase
    {
        #region Parameter
        [Parameter]
        ...
        #region variables
        WithdrawRequestDetailVM getWithdrawCaseRequest;
        ...
        protected async Task WithdrawRequestDetailbyId(Guid WithdrawRequestId, int CommunicationTypeId)
        {
            try
            {
                var response = await caseRequestService.GetRequestWithdrawDetailById(WithdrawRequestId, CommunicationTypeId);
                if (response.IsSuccessStatusCode)
                {
                    getWithdrawCaseRequest = (WithdrawRequestDetailVM)response.ResultData;
                ...
                if (dialogResponse == true)
                {
                    if (isRejected == true)
                    {
                        ...
                        if (dialogResult != null)
                        {
                            ...
                            var response = await caseRequestService.UpdateWithDrawCaseRequest(request, isRejected);
                            if (response.IsSuccessStatusCode)
                            {
        ...
        //<History Author = 'Hassan Abbas' Date='2022-09-30' Version="1.0" Branch="master">Populate Departments</History>
        ...
        protected async Task LoadAuthorityLetter()
        {
            try
            {
                ...
                var authorityLetter = attachments?.Where(t => t.AttachmentTypeId == (int)AttachmentTypeEnum.WithdrawRequest).FirstOrDefault();
