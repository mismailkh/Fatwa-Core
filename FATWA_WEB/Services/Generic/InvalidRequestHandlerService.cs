...
namespace FATWA_WEB.Services.General
{
    /// <summary>
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (badRequestResponse?.InnerException != null && badRequestResponse.InnerException.ToLower().Contains("violation of unique key"))
                    {
                        ...
                        {
                            Severity = NotificationSeverity.Error,
                            Detail = string.IsNullOrEmpty(alreadyDataExistMessage) ? "Save_failed_duplicate_data_detected" : _translationState.Translate(alreadyDataExistMessage),
                            Style = "position: fixed !important; left: 0; margin: auto; "
                        });
                    }
                    else
