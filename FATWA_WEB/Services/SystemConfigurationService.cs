...
namespace FATWA_WEB.Services
{
    ...
    public class SystemConfigurationService
    {
        ...
        {
            var request = new HttpRequestMessage(HttpMethod.Get, _config.GetValue<string>("api_url") + "/SystemConfiguration/GetUserDetailByUsingEmail?email=" + email);
        ...
        public async Task<ApiCallResponse> GetUserDetailByUsingEmailForPasswordCheck(string email)
        {
            try
            {
                var request = new HttpRequestMessage(HttpMethod.Get, _config.GetValue<string>("api_url") + "/SystemConfiguration/GetUserDetailByUsingEmailForPasswordCheck?email=" + email);
                //request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", await browserStorage.GetItemAsync<string>("Token"));
                var response = await new HttpClient().SendAsync(request);
        ...
        public async Task<ApiCallResponse> GetUserGroupDetailByUsingGroupId()
        {
            try
            {
                var request = new HttpRequestMessage(HttpMethod.Get, _config.GetValue<string>("api_url") + "/SystemConfiguration/GetUserGroupDetailByUsingGroupId");
                //request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", await browserStorage.GetItemAsync<string>("Token"));
                var response = await new HttpClient().SendAsync(request);
                if (response.IsSuccessStatusCode)
                {
        ...
        public async Task<ApiCallResponse> LockUserAccount(string email)
        {
            ...
            {
                // user.IsLocked = true;
                var request = new HttpRequestMessage(HttpMethod.Get, _config.GetValue<string>("api_url") + "/SystemConfiguration/LockUserAccount?email=" + email);
                //var postBody = user;
        ...
        public async Task<ApiCallResponse> UserAccountAccessFailCount(string email, int wrongCount)
        {
            try
            {
                ...
                var request = new HttpRequestMessage(HttpMethod.Get, _config.GetValue<string>("api_url") + "/SystemConfiguration/UserAccountAccessFailCount?email=" + email + "&wrongCount=" + wrongCount);
                //var postBody = user;
                //request.Content = new StringContent(System.Text.Json.JsonSerializer.Serialize(postBody), Encoding.UTF8, "application/json");
                //request.Headers.Authorization = new AuthenticationHeaderValue("Bearer", await browserStorage.GetItemAsync<string>("Token"));
                var response = await new HttpClient().SendAsync(request);
