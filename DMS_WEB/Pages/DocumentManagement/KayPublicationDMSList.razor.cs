...
{
    public partial class KayPublicationDMSList : ComponentBase
    {
        #region Varriable
        ...
        protected async Task OnLoadData(LoadDataArgs dataArgs)
        {
            if (dataArgs.OrderBy == "")
            {
                ...
                {
                    spinnerService.Show();
                    advanceSearchVM.PageSize = dataArgs.Top;
                    advanceSearchVM.PageNumber = (dataArgs.Skip / dataArgs.Top) + 1;
                    advanceSearchVM.FromLegalLegislationForm = false;
                    var response = await fileUploadService.GetKayDocumentsList(advanceSearchVM);
                    spinnerService.Hide();
                    if (response.IsSuccessStatusCode)
                    {
                        var result = (IEnumerable<DMSKayPublicationDocumentListVM>)response.ResultData;
                        DocumentList = result.Where(x => !x.IsFullEdition);
                        FilteredDocumentList = result.Where(x => !x.IsFullEdition);
                        FullEditionDocumentList = result.Where(x => x.IsFullEdition);
        ...
        {
            advanceSearchVM = new KayDocumentListAdvanceSearchVM();
        ...
        protected async Task ViewAttachement(DMSKayPublicationDocumentListVM theUpdatedItem)
        {
            try
            {
